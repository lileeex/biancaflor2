...
const MainNavbar = ({ onDrawerToggle }: NavbarProps) => {
  ...
  const routeName = pathSegments.length > 0 ? pathSegments.pop() : 'Bienvenidos';
        ...
        >
          ...
          <Stack direction="row" gap={1} sx={{ display: { xs: 'flex', md: 'none' } }}>
            <Link href="/" sx={{ display: 'flex', p: 0.5 }}>
              <Image src="Biancaflor2.svg" alt="logo" sx={{ width: 25 }} />
            </Link>
            <IconButton onClick={onDrawerToggle} sx={{ display: { md: 'none' } }}>
              <IconifyIcon icon="mingcute:menu-line" color="primary.darker" width={20} />
            </IconButton>
          </Stack>
